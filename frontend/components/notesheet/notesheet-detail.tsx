          ...
          <CardHeader className="flex flex-row flex-wrap items-start justify-between gap-3 border-b border-border pb-4">
            <div className="flex flex-col gap-2">
              <div className="flex flex-wrap items-center gap-2">
                ...
                {noteSheet.draftSource && (
                  <Badge
                    variant="outline"
                    title={
                      noteSheet.draftSource === "template"
                        ? noteSheet.draftError ?? "The LLM was unavailable, so a static template was used."
                        : `Draft generated live by ${noteSheet.draftSource}.`
                    }
                    className={
                      noteSheet.draftSource === "template"
                        ? "border-amber-500/40 bg-amber-500/10 font-mono text-[10px] tracking-wide text-amber-700 dark:text-amber-400"
                        : "border-primary/35 bg-primary/[0.07] font-mono text-[10px] tracking-wide text-primary"
                    }
                  >
                    {noteSheet.draftSource === "template" ? "template fallback" : `drafted by ${noteSheet.draftSource}`}
                  </Badge>
                )}
