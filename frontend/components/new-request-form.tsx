...
import { generateDraft } from "@/lib/api/notesheets"
...
export function NewRequestForm() {
  ...
  async function handleGenerate() {
    ...
    try {
      ...
      setDraft(result)
      if (result.draftText && result.precedentIds.length === 0) {
      ...
      toast.error(`Could not reach the drafting service. ${message}`)
    } finally {
      setGenerating(false)
    }
  }
    ...
    <div className="flex flex-col gap-6">
      ...
      {draft && !generating && (
        <NoteSheetDetail
          ...
          approvalChain={
            <ApprovalStepper stages={draft.approvalStages} />
          }
          headerAction={
            <Button size="sm" onClick={handleSubmitForApproval} disabled={submitting || draft.status !== "Draft"}>
